...
export default function ExtensionCallbackPage() {
    ...
    return (
        <main className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
            <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-800/50 p-8 text-center backdrop-blur-sm">
                {/* Logo */}
                ...
                {status === "loading" && (
                    <>
                        <div className="mb-4 flex h-16 w-16 mx-auto items-center justify-center rounded-full bg-blue-500/20">
                            <div className="h-8 w-8 animate-spin rounded-full border-2 border-blue-400 border-t-transparent" />
                        </div>
                        <h1 className="mb-2 text-xl font-semibold text-white">Connecting...</h1>
                    </>
                )}

                {status === "success" && (
                    <>
                        <div className="mb-4 flex h-16 w-16 mx-auto items-center justify-center rounded-full bg-green-500/20">
                            <svg className="h-8 w-8 text-green-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <polyline points="20 6 9 17 4 12" />
                            </svg>
                        </div>
                        <h1 className="mb-2 text-xl font-semibold text-white">Connected!</h1>
                        <p className="mb-4 text-sm text-slate-400">{message}</p>
                        <p className="text-sm text-slate-500">You can close this tab and reopen the extension.</p>
                    </>
                )}
